...
async function bridgeApiCookies(response: Response): Promise<void> {
 ...
 *
 * The page renders a Server Component form bound to this action — no client
...
export async function loginAction(formData: FormData): Promise<void> {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      if (body?.error?.code) code = body.error.code.toLowerCase()
    } catch {
...
export async function logoutAction(): Promise<void> {
  ...
  try {
    ...
    // send the visitor to the sign-in screen — being stuck on a logged-in page
 ...
 * invalidated the whole module — the two real actions stopped being
