...
/**
 * Creating a project from the wizard, for whichever portal is asking.
 *
 * ── WHY THIS IS NOT IN EITHER portal's `actions.ts`
 *
 * The sequence is the same for both: validate the wizard's own rules
 * server-side, POST the project, find its default build, PATCH the build with
 * the build-level fields, and map every failure onto a code the wizard can
 * word. Admin adds an organisation and a priority to the create body; that is
 * the whole difference. Copied, the two would drift the way the two forms
 * already had — one portal's validation fixed and the other's not.
 *
 * Not a Server Action itself: each portal exports its own thin action, so the
 * role gate stays where the route is and only that portal's paths are used.
 */

export interface WizardCreateConfig {
  /** Where the wizard lives, for the redirects that carry errors back. */
  wizardPath: string
  /** Where a created project is shown. */
  projectHref: (projectId: string) => string
  /** Paths to revalidate once a project exists. */
  revalidate: readonly string[]
  /** Extra project-level fields. Admin sends organisationId and priority. */
...
function carryBack(formData: FormData, errorCode: string, detail = ''): string {
  const keep = [
    ...
    'instructions',
    'specialRequirements',
    // Admin-only, and carried back for the same reason as everything else:
    // a rejected create must not empty the form.
    'organisationId',
    'priority',
  ] as const
...
export async function createProjectFromWizard(
  formData: FormData,
  config: WizardCreateConfig,
): Promise<void> {
  const WIZARD = config.wizardPath
  ...
  if (buildId) {
    try {
      ...
      redirect(`${config.projectHref(project.id)}?notice=build-incomplete`)
    }
  }

  for (const path of config.revalidate) revalidatePath(path)
  redirect(`${config.projectHref(project.id)}?notice=created`)
}
