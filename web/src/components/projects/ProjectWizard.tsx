...
import { FormBackButton } from '@/components/projects/BackButton'

/**
 * The project wizard, shared by the Customer and Admin portals.
 *
 * ── WHY ONE COMPONENT AND NOT TWO PAGES
 *
 * Admin had its own flat, single-page create form. It collected twelve fields
 * where this collects twenty, and — more to the point — it never touched the
 * project's first build, so an admin-created project arrived with an empty
 * build somebody had to fill in afterwards. Two forms for one job had already
 * drifted that far apart, and would have kept drifting.
 *
 * So the portals differ in exactly three things, all passed in: where the
 * wizard lives, which action creates the project, and whether the reader may
 * choose an organisation and a priority. Everything else is this file.
 *
 * Admin-only by design: a customer has exactly one organisation (the API
 * resolves it from their membership) and priority is internal triage, which
 * is why neither is offered to them.
 *
...
export interface WizardParams {
  ...
  /** Admin only — see `ProjectWizardProps`. */
...
function stepHref(step: Step, params: WizardParams, basePath: string): string {
...
export interface ProjectWizardProps {
  /** Where the wizard lives, so each step can navigate to the next. */
  basePath: string
  /** The portal's own upload route — each has its own role check. */
  uploadPath: string
  root: { label: string; href: string }
  /** The portal's projects list, for the breadcrumb. */
  projectsHref: string
  /** The portal's create action. Both end up calling the same two endpoints. */
  action: (formData: FormData) => void | Promise<void>
  params: WizardParams
  /**
   * Organisations the reader may create under. Admin only — a customer's
   * organisation is resolved from their membership by the API, so offering a
   * choice would be offering one they do not have.
   */
  organisations?: readonly { id: string; name: string }[]
  /** Priority options. Admin only, for the same reason: internal triage. */
...
export async function ProjectWizard({
  ...
  return (
    <DetailShell
      root={root}
      crumbs={[{ label: 'Projects', href: projectsHref }, { label: 'New test' }]}
      eyebrow="Delivery"
      title="Set up a test"
    ...
    >
      ...
      */}
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <li key={s.value} aria-current={state === 'current' ? 'step' : undefined}>
                {state === 'done' ? (
                  <a href={stepHref(s.value, params, basePath)} style={{ textDecoration: 'none' }}>
                    {body}
                  </a>
                ) : (
                  body
        ...
        >
          <div style={FIELD_GRID}>
            {SUBJECTS.map((subject) => (
              <form key={subject.value} method="get" action={basePath}>
                ...
                <Carry name="organisationId" value={params.organisationId} />
                <Carry name="priority" value={params.priority} />
      ...
      {step === 'general' ? (
        <form method="get" action={basePath} style={FORM_STYLE}>
          ...
          >
            <div style={FORM_STYLE}>
              {/*
                Admin only, and rendered first because it decides who the
                project belongs to — a question the rest of the form assumes
                an answer to. A customer sees neither: the API resolves their
                organisation from their membership, and priority is internal
                triage rather than something the person requesting the test
                should be setting.
              */}
              {organisations || priorities ? (
                <div style={FIELD_GRID}>
                  {organisations ? (
                    <Field label="Organisation" htmlFor="organisationId" required>
                      <Select
                        id="organisationId"
                        name="organisationId"
                        required
                  ...
                  {priorities ? (
                    <Field label="Priority" htmlFor="priority">
                      <Select
                        id="priority"
                        name="priority"
                        defaultValue={params.priority ?? 'NORMAL'}
                        options={priorities}
                      />
                    </Field>
                  ) : null}
      ...
      {step === 'filters' ? (
        <form action={action} style={FORM_STYLE}>
          ...
          >
            <FormBackButton action={basePath} step="details">
              Back
            </FormBackButton>
