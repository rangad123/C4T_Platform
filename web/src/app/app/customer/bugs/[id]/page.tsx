...
export default async function CustomerBugDetailPage({
      ...
      >
        {status === 403 ? (
          <EmptyState
            ...
            description="This bug doesn't belong to your organisation."
            action={
  ...
  return (
    <DetailShell
      ...
      subtitle={
        <>
          {bug.reference} · reported {formatTimestamp(bug.createdAt)} on{' '}
          <Link href={projectHref} style={LINK_STYLE}>
            {bug.project.title}
          </Link>
        </>
            ...
            <Modal open={statusModalOpen} closedHref={closedHref} title="Move status">
              <TrackedForm action={moveBugStatus} style={FORM_STYLE}>
                ...
                <Field
                  label="Note"
                  htmlFor="note"
                  hint="Recorded against the move. Required when rejecting a bug or marking it won’t fix."
                >
                  <Textarea
                    id="note"
                    name="note"
            ...
            >
              ...
              <Field label="Add a comment" htmlFor="body" required>
                <Textarea
                  ...
                  required
                  maxLength={5000}
                  placeholder="Ask a question, or add context for the tester."
                />
              </Field>
              <div>
                <SubmitButton
